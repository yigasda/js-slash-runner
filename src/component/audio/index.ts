...
async function openUrlManagerPopup(typeKey: 'bgmurl' | 'ambienturl') {
  ...
  function renderUrl(container: JQuery<HTMLElement>, url: string) {
    ...
    urlHtml.find('.delete_url').on('click', async function () {
      const confirmDelete = await callGenericPopup('이 링크를 삭제하시겠습니까?', POPUP_TYPE.CONFIRM);
  ...
  if (enable) {
    ...
    if (bgmUrl.length > 0) {
      ...
      try {
        ...
        throw new Error('[Audio] 음악 재생 실패: 유효한 소스가 제공되지 않았습니다.');
      }
    }
    if (ambientUrl.length > 0) {
      ...
      try {
        ...
        throw new Error('[Audio] 음향 효과 재생 실패: 유효한 소스가 제공되지 않았습니다.');
      }
    }
  } else {
...
async function openUrlImportPopup(): Promise<string[] | null> {
  const input = (await callGenericPopup('가져올 네트워크 오디오 링크를 입력하세요 (행마다 하나씩).', POPUP_TYPE.INPUT, '')) as string | null;
