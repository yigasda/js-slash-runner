...
function connect_socket(url: string): void {
  ...
  socket.on('connect_error', (error: Error) => {
    ...
    if (socket.active) {
      if (getSettingValue('listener.enable_echo')) {
        toastr.error(`태번 도우미의 실시간 리스너 기능 연결 중 오류가 발생했습니다. 재연결을 시도합니다...\n${error.name}: ${error.message}`);
      }
      console.error(`${error.name}: ${error.message}${error.stack ?? ''}`);
    } else {
      if (getSettingValue('listener.enable_echo')) {
        toastr.error(`태번 도우미의 실시간 리스너 기능 연결 중 오류가 발생했습니다. 수동으로 다시 연결해 주세요!\n${error.name}: ${error.message}`);
      }
      console.error(`${error.name}: ${error.message}${error.stack ?? ''}`);
    }
  });
  ...
  socket.on('disconnect', (reason, details) => {
    if (getSettingValue('listener.enable_echo')) {
      toastr.warning(`태번 도우미 실시간 리스너 연결이 끊어졌습니다: ${reason}`);
    }
    toggle_status(false);
