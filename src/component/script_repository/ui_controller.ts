...
export class UIController {
  ...
  private showEmptyScriptListTip(type: ScriptType): void {
    ...
    if (container.find('small').length === 0) {
      container.append('<small>등록된 스크립트가 없습니다.</small>');
    }
  }
  ...
  private async renderScript(script: Script, type: ScriptType): Promise<void> {
    ...
    scriptHtml.find('.delete-script').on('click', async () => {
      const confirm = await callGenericPopup('이 스크립트를 삭제하시겠습니까?', POPUP_TYPE.CONFIRM);
    ...
    scriptHtml.find('.add-script').on('click', async () => {
      ...
      if (existing_script) {
        const input = await callGenericPopup(
          `가져오려는 스크립트 '${script.name}'이(가) 스크립트 라이브러리의 '${existing_script.name}'과(와) ID가 동일합니다. 가져오시겠습니까?`,
          POPUP_TYPE.TEXT,
          '',
          {
            okButton: '기존 스크립트 덮어쓰기',
            cancelButton: '취소',
            customButtons: ['새 스크립트로 저장'],
          },
        );
      ...
      toastr.success(`"${newScript.name}" 스크립트가 ${target === ScriptType.GLOBAL ? '전역' : '캐릭터'} 스크립트 라이브러리에 추가되었습니다.`);
    });
  ...
  private async openScriptEditor(type: ScriptType, scriptId?: string): Promise<void> {
    ...
    $editorHtml.find('#add-button-trigger').on('click', () => {
      ...
      const $buttonContent = $(`<div class="button-item" id="${buttonId}">
        ...
        <input class="text_pole button-name" type="text" id="text-${buttonId}" placeholder="버튼 이름">
        <div class="delete-button menu_button interactable" data-index="${buttonIndex}">
          <i class="fa-solid fa-trash"></i>
        </div>
      </div>`);
    ...
    if (!script) {
      ...
      if (wasEnabled) {
        try {
          ...
          toastr.error(`스크립트 재시작 실패: ${script.name}`);
        }
      }
    }
  }
  ...
  private async openVariableEditor(): Promise<void> {
    if (!this_chid) {
      toastr.error('저장 실패: 현재 캐릭터가 설정되어 있지 않습니다.');
      return;
    }
    ...
    for (const [key, value] of Object.entries(existingVariables)) {
      const $variableRow = $(`
        <div class="variable-row flex-container mb-2" style="gap:5px;">
          <input type="text" class="text_pole variable-key" style="width:150px;" value="${key}" placeholder="변수 이름">
          <input type="text" class="text_pole variable-value" style="flex-grow:1;" value="${value}" placeholder="변수 값">
          <div class="menu_button delete-variable">삭제</div>
        </div>
      `);
    ...
    $editorHtml.find('#add-variable').on('click', function () {
      const $variableRow = $(`
        <div class="variable-row flex-container mb-2" style="gap:5px;">
          <input type="text" class="text_pole variable-key" style="width:150px;" placeholder="변수 이름">
          <input type="text" class="text_pole variable-value" style="flex-grow:1;" placeholder="변수 값">
          <div class="menu_button delete-variable">삭제</div>
        </div>
      `);
    ...
    toastr.success('캐릭터 스크립트 변수 저장 완료');
  }
