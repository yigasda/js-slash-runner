        ...
        if (!disableLoading) {
          ...
          loadingTimeout = setTimeout(() => {
            ...
            if ($loadingText.length) {
              $loadingText.text('로딩 시간이 너무 길면 네트워크를 확인해 주세요.');
            }
          }, 10000);
...
async function onDepthInput(value: string) {
  ...
  if (processDepth < 0) {
    toastr.warning('처리 깊이는 음수일 수 없습니다.');
...
function addToggleButtonToCodeBlock($pre: JQuery<HTMLElement>) {
  ...
  const $toggleButton = $(
    '<div class="code-toggle-button" title="[태번 도우미 - 렌더러 - 렌더링 최적화]를 끄면 이 접기 기능이 비활성화됩니다.">코드 블록 표시</div>',
  );
...
function addRenderQuickButton() {
  ...
  <div id="tavern-helper-render-container" class="list-group-item flex-container flexGap5 interactable">
      ...
      <span id="tavern-helper-render-text">${isRenderEnabled ? '프론트엔드 렌더링 끄기' : '프론트엔드 렌더링 켜기'}</span>
  </div>`);
  buttonHtml.css('display', 'flex');
  $('#extensionsMenu').append(buttonHtml);
  $('#tavern-helper-render-container').on('click', async function () {
    $('#tavern-helper-render-text').text(!isRenderEnabled ? '프론트엔드 렌더링 끄기' : '프론트엔드 렌더링 켜기');
