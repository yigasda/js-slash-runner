...
export class VariableView implements IDomUpdater {
  ...
  public refreshVariableCards(type: VariableType, variables: VariableItem[]): void {
    ...
    // 更新标签文本
    this.container.find('.variable-type-label').text(`${type}변수`);
    ...
    if (variables.length === 0) {
      $variableList.html('<div class="empty-state"><p>등록된 변수가 없습니다.</p></div>');
      return;
    }
  ...
  private async renderMessageVariablesByFloor(
    ...
    try {
      ...
      if (!controller || !controller.model) {
        throw new Error('변수 모델을 찾을 수 없습니다.');
      }
      ...
      if (isNaN(minFloor) || isNaN(maxFloor)) {
        $container.html('<div class="empty-state"><p>유효한 층 범위 필터 조건을 설정해주세요.</p></div>');
      ...
      if (floorIds.length === 0) {
        $container.html('<div class="empty-state"><p>현재 필터 조건에서는 아무 층도 찾을 수 없습니다.</p></div>');
        return { cancelled: false };
      }
      ...
      if ($container.children().length === 0) {
        $container.html('<div class="empty-state"><p>현재 필터 조건에서는 아무 층 변수도 찾을 수 없습니다.</p></div>');
      }

      return { cancelled: false };
    } catch (error: any) {
      console.error(`[VariableView] 渲染楼层变量失败:`, error);
      $container.html(`<div class="empty-state"><p>변수 렌더링 오류: ${error.message}</p></div>`);
      return { cancelled: false };
    }
  }
  ...
  public updateVariableCard(name: string, value: any, isNewCard: boolean = false): boolean {
    try {
      ...
      toastr.error(`변수 카드 "${name}" 업데이트에 실패했습니다: ${error}`);
      return false;
    }
  }
    ...
    const content = $(`
      <div>
        ...
        <div class="floor-input-dialog">
          <input type="number" id="floor-input" min="0" placeholder="층 번호를 입력하세요." />
          <div id="floor-input-error" class="floor-filter-error" style="display: none">유효한 층 번호를 입력하세요.</div>
        </div>
      </div>
    `);
    ...
    const content = $(`
      <div>
        ...
        <div class="variable-type-options">
          <div data-type="string"><i class="fa-regular fa-font"></i> 문자열</div>
          <div data-type="number"><i class="fa-regular fa-hashtag"></i> 숫자</div>
          <div data-type="boolean"><i class="fa-regular fa-toggle-on"></i> 불리언</div>
          <div data-type="array"><i class="fa-regular fa-list"></i> 배열</div>
          <div data-type="object"><i class="fa-regular fa-code"></i> 객체</div>
        </div>
      </div>
    `);
    ...
    this.dialog = $(`
      <div class="variable-manager-dialog">
        <div class="dialog-header">
          <div class="dialog-title">변수관리자</div>
          <div class="dialog-controls">
            <button class="dialog-toggle-btn" title="내용 접기/펼치기"><i class="fa-solid fa-chevron-up"></i></button>
    ...
    const content = $(`
      <div>
        ...
        <div class="key-input-dialog">
          <input type="text" id="key-input" placeholder="키 이름을 입력하세요." />
          <div id="key-input-error" class="input-error" style="display: none">유효한 키 이름을 입력하세요.</div>
        </div>
      </div>
    `);
